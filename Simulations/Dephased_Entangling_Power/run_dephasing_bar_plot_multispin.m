...
%
%Input: lambda0 \in [0,1] (lambda0=1 no dephasing)
%--------------------------------------------------------------------------
...
for jj=1:cases
  ...
  for ll=1:length(theta_coeff)
      ...
      theta = theta_coeff(ll);
      temp  = SubClass_Ent_and_Fid;
      temp  = temp.dephased_epM_CR(n0,n1,phi0,phi1,lambda0,lambda1,theta,t,N);
