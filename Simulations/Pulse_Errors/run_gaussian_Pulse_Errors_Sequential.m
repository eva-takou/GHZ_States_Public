...
parfor ll=1:MaxRep
   ...
   for jj=1:cases
       ...
       At = D.Value.A_Target{jj};
       Bt = D.Value.B_Target{jj};
       t  = D.Value.Opt_Unit_Times{jj};
       N  = D.Value.Opt_Iters{jj};
       ...
       test = SuperClass_Sequences(wL,At,Bt,s0,s1,Nnuc,k,1);
       test = test.XY2_error_variance(t,N,variance);
       U    = test.Uevol;
